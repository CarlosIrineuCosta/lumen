...
## RELATED DECISION: PostgreSQL vs Firestore Architecture (2025-08-04)

**Context:** Critical architectural mismatch discovered during upload functionality implementation.

**Problem:** 
- PostgreSQL Models (`/models/photo.py`): SQLAlchemy with `CreatePhotoRequest` requiring `image_url: str`
- Firestore Service (`/services/photo_service.py`): Using `firestore.client()` with file upload handling
- API Endpoint (`/api/endpoints/photos.py`): Expects file upload but validates with PostgreSQL model

**Upload Failure:** `1 validation error for CreatePhotoRequest image_url Field required`

**Decision:** PostgreSQL (Cloud SQL) over Firestore
...
**Status:** Implemented - PostgreSQL architecture is in production use