...
class UserService:
    ...
    async def create_user_profile(self, firebase_user: AuthUser, request: CreateUserRequest) -> UserProfileFull:
        ...
        try:
            ...
            existing_user = db.query(User).filter(User.id == firebase_user.uid).first()
            if existing_user:
                raise ValueError("User profile already exists")
            
            # Create new user
            user = User(
                id=firebase_user.uid,  # Firebase UID is the primary key
    ...
    async def get_user_profile(self, uid: str, include_private: bool = False) -> Optional[UserProfileFull]:
        """Get user profile by Firebase UID (which is now the primary key)"""
        db = SessionLocal()
        try:
    ...
    async def get_deletion_summary(self, uid: str) -> Dict[str, Any]:
        """Get summary of what will be deleted before account deletion"""
        db = SessionLocal()
        try:
            user = db.query(User).filter(User.id == uid).first()
            if not user:
            ...
            # Import Photo model to count photos
            from ..models.photo import Photo
            
            # Count photos
            photo_count = db.query(Photo).filter(Photo.user_id == uid).count()
            
            # Get list of photo files to delete from GCS
            photos = db.query(Photo).filter(Photo.user_id == uid).all()
            photo_files = []
            for photo in photos:
                if photo.storage_path:
                    photo_files.append(photo.storage_path)
            ...
            return {
                ...
                "profile_image": user.profile_image_url if user.profile_image_url != user.email else None,
                "account_created": user.created_at.isoformat()
            }
        finally:
            db.close()
    
    async def delete_user_account(self, uid: str) -> None:
        """
        ...
        - Profile image from Google Cloud Storage
        """
        db = SessionLocal()
        try:
            user = db.query(User).filter(User.id == uid).first()
            ...
            # Get all photos for deletion from GCS
            photos = db.query(Photo).filter(Photo.user_id == uid).all()
            
            # Delete photo files from Google Cloud Storage
            bucket = self.storage_client.bucket(self.bucket_name)
            deleted_files = []
            failed_deletions = []
            ...
            for photo in photos:
                if photo.storage_path:
                    try:
                        ...
                        blob.delete()
                        deleted_files.append(photo.storage_path)
                    except Exception as e:
            ...
            # Delete profile image from GCS if it's a custom upload
            if user.profile_image_url and not user.profile_image_url.startswith('https://lh3.googleusercontent.com'):
                try:
                    # Extract storage path from URL
                    # Format: https://storage.googleapis.com/bucket-name/path/to/file
                    if 'storage.googleapis.com' in user.profile_image_url:
                        parts = user.profile_image_url.split('/')
                        if len(parts) > 4:
                            storage_path = '/'.join(parts[4:])  # Skip protocol, domain, bucket
            ...
            # Delete all photos from database (CASCADE should handle relationships)
            db.query(Photo).filter(Photo.user_id == uid).delete()
            
            # Delete user from database (CASCADE should handle other relationships)
            db.delete(user)
            ...
            if failed_deletions:
                print(f"Warning: Failed to delete some files for user {uid}: {failed_deletions}")
